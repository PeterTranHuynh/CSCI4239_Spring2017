...
//  X Y Z Positions
void Hw1opengl::setX(int pos)
{
    obj->setTranslate((float)pos, obj->getY(), obj->getZ());
    update();
}

void Hw1opengl::setY(int pos)
{
    obj->setTranslate(obj->getX(), (float)pos, obj->getZ());
    update();
}

void Hw1opengl::setZ(int pos)
{
    obj->setTranslate(obj->getX(), obj->getY(), (float)pos);
    update();
}
...
{
   QMessageBox::critical(this,"Hw1opengl",message);
   QApplication::quit();
}
