...
float inverseFlowingNoise2D(vec2 vector)
{	
    //  i is used for for loop
    int i = 0;
	//  Multiply by reflective matrix
	...
	if (mode == 1) noiseVariance = noise(vector);
	if (mode == 2) noiseVariance = snoise(vector);
	
	//  Loop for n setting and vector
	for(i = 1; i < 7; i++)
	{
		n += abs((noiseVariance - 0.5) * 2.0) / d;
		d *= 3.0;
		vector *= 5.0 * m2;
	}
	return n;
...
float inverseFlowingNoise3D(vec3 vector)
{	
    int i = 0;
	mat3 m3 = mat3(0.80,  0.60,  0.40, 
	               0.20,  0.00, -0.20, 
				  -0.40, -0.60, -0.80);
	float d = 2.0;
	float n = 0.0;
	...
	float noiseVariance = 0.0;
	if (mode == 3) noiseVariance = noise(vector);
	if (mode == 4) noiseVariance = snoise(vector);
...
float inverseFlowingNoise4D(vec4 vector)
{	
    int i = 0;
	mat4 m4 = mat4(0.80,  0.70,  0.60,  0.50, 
	               0.40,  0.30,  0.20,  0.10, 
				   0.00, -0.10, -0.20, -0.30, 
				  -0.40, -0.50, -0.60, -0.70);
	float d = 2.0;
	...
	float noiseVariance = 0.0;
	if (mode == 5) noiseVariance = noise(vector);
	if (mode == 6) noiseVariance = snoise(vector);
...
float staticOverlay(vec3 vector)
{
    return fract(tan(dot(vector.xyz ,vec3(12.9898, 78.2337, 38.4713))) * 43758.5453);
}
...
void main(void)
{
   ...
   //  2D classic and simplex noise
   if (mode == 1 || mode == 2)
	  n = inverseFlowingNoise2D(T2 * 0.5 + T4.w) * dot(T2 * 2.0, T2 * 0.5);
   //  3D classic and simplex noise
   else if (mode == 3 || mode == 4)
	  n = inverseFlowingNoise3D(T3 * 0.5 + T4.w) * dot(T3 * 2.0, T3 * 0.5);
   //  4D classic and simplex noise
   else if (mode == 5 || mode == 6)
	  n = inverseFlowingNoise4D(T4) * dot(T4 * 2.0, T4 * 0.5);
   else
      n = 0.0;

   //  Rescale between 0-1, set to 1
   n = 1.0 * n + 1.0;
   ...
   {
     // Color based Noise
	 vec3 color = ((T3 * staticOverlay(T3))  / (1.0 - n)) * pow(LightIntensity, 0.5) * 2.0;
	 gl_FragColor = vec4(sqrt(abs(color)), 1.0);
   }
}