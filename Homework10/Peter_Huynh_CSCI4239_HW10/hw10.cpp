 ...
 *  Title:      HW10 - Homework 10: GPU Computing
 *  Course:     CSCI 4239
 *  Semester:   Spring 2017
 *  File Notes:
 *    Main C++ file for OpenCL
 *******************************
 *
 ...
 * host
 */
void AxBh(float C[], const float A[], const float B[], const float D[], unsigned int n)
{
	// SETS A BASIC SIMPLE FILTER FOR THE COMBINE ORIENTATION, Used to rank robot positions
	//  CALCULATING ONE OF THREE LOCATION ORIENTATION VECTOR FILTERD BASE OFF ROLL PITCH YAW
	//  ROLL FILTER FOR A Kalmen Filter
   for (unsigned int i=0;i<n;i++)
      C[i] = sin(A[i]) * tan(B[i]) + cos(D[i]);
}
 ...
 * C = A * B -- device
 */
void AxBd(float Ch[],float Ah[],float Bh[],float Dh[],const unsigned int Bw,const unsigned int Bn)
{
   ...
   cl_mem Dd = clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,N,Dh,&err);
   if (err) Fatal("Cannot create and copy B from host to device\n");
   ...
   if (clSetKernelArg(kernel,2,sizeof(cl_mem),&Dd)) Fatal("Cannot set kernel parameter Dd\n");
...
int main(int argc, char* argv[])
{
   ...
   if (!Ah || !Bh || !Ch || !Dh || !Rh) Fatal("Cannot allocate host memory\n");
